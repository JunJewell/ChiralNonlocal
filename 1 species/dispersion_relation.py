...
class dispersion():
    ...
    def __init__(self, k, angle, rho, U, mu, xi, kernel_name):
    ...
    def dispersion_relation(self):
        return  ( -self.rho - (self.k**2) + 
                 (np.cos(self.angle)*2*np.pi*self.U*(1-self.U)*self.mu/self.xi)*self.F )
...
if __name__ == '__main__':
    ...
    lambda_points = dispersion(k_points, angle, rho, U, mu, xi, kernel_name).dispersion_relation()
