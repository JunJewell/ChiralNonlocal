...
class plotter():
    ...
    def __init__(self, angle, rho, U, mu, xi, kernel_name, L_x, L_y, mesh_points_x, mesh_points_y, solution_y, solution_t, 
                  folder_tag, file_tag, use_PGF=True, save=True, raster_heatmap=None, num_heatmap_ticks=6):
    ...
    def heatmap(self, time_index=-1):
        ...
        if (self.save):
            fig.savefig("%s/%s_heatmap_t=%.2f.pdf" %(self.folder_tag, self.file_tag,
    ...
    def animation(self, sample_rate=100):
        ...
        if (self.save):
            f = r"%s/%s_anim.mp4" %(self.folder_tag, self.file_tag)
    ...
    def dispersion_relation(self, sample_t=1, xlims=None, ylims=None):
        ...
        theory_lambda_points = dispersion(theory_k_points, self.angle,
                                                       self.rho, self.U,
                                                       self.mu, self.xi,
        ...
        if (self.save):
            fig_disp.savefig("%s/%s_dispersionRelation.pdf" %(self.folder_tag, self.file_tag))