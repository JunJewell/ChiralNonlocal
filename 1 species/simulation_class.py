...
class nonlocal_react_advect_diffuse_system():
    ...
    def __init__(self, U, angle, rho, xi, mu, L_x, L_y, kernel_name, mesh_points_x, time_span, time_evaluations, integrator_method="BDF",  rtol=1e-11, atol=1e-11, initial_conditions=None, seed=15):
        
        #Equation parameters
        
        self.angle = angle    #deflection angle, relative to separation, of nonlocal interaction
    ...
    def circular_weighted_kernel(self):
        ...
        kernel_y = (np.cos(self.angle)*y/distance_from_centre) +(np.sin(self.angle)*x/distance_from_centre)
        kernel_y[ np.where(distance_from_centre>radius_length) ] = 0
        
        kernel_x = (np.cos(self.angle)*x/distance_from_centre) - (np.sin(self.angle)*y/distance_from_centre)
    ...
    def exponential_kernel(self):
        ...
        kernel_x = ((np.cos(self.angle)*x/distance_from_centre) - (np.sin(self.angle)*y/distance_from_centre)) * (np.exp(-distance_from_centre/self.xi))
        kernel_y = ((np.cos(self.angle)*y/distance_from_centre) +(np.sin(self.angle)*x/distance_from_centre)) * (np.exp(-distance_from_centre/self.xi))
